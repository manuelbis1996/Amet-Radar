...
(async () => {
  ...
  await page.route('**/rest/v1/rpc/**', r => {
    ...
    rpcs.push({ fn, args });
    // Desde v14.0 publicar NO es un POST a /rest/v1/reports sino la RPC
    // create_report (anti-spam del lado del servidor). Se normaliza a la
    // forma del record para que los chequeos de abajo sigan valiendo igual.
    if(fn === 'create_report'){
  ...
  check('se publicó exactamente 1 reporte (por rpc/create_report)', inserts.length === 1, 'inserts=' + inserts.length);
  const rep = inserts[0] || {};
