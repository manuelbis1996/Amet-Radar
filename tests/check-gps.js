...
async function nuevaPagina(browser, geo){
  ...
  // v14.0: publicar pasa por rpc/create_report. Sin esta ruta la llamada
  // saldría a la red de verdad y la publicación fallaría.
...
(async () => {
  ...
  let enviado = null;
  page.on('request', r => {
    // v14.0: el punto publicado viaja en los argumentos de create_report.
    if(r.method() === 'POST' && /rest\/v1\/rpc\/create_report/.test(r.url())){
      try{
        const a = JSON.parse(r.postData() || '{}');
        enviado = { lat: a.p_lat, lng: a.p_lng };
      }catch(e){}
    }
  });
