...
| `check-acerca.js` | La hoja "Acerca de" (v17.8) y **que la topbar entre en la pantalla** (v17.10): que las dos entradas (botón ⓘ de la topbar y enlace de la bienvenida) la abran y que se cierre igual que cualquier hoja; que diga las tres cosas que tiene que decir —autoría y contacto, qué datos se guardan, y que los reportes no están verificados—; que la topbar de **tres** botones siga entrando a 320 px sin solaparse con la marca; y que `admin.html` lleve `noindex`. La medición de la topbar se hace en **320/360/390/412 px y con 128 reportes en pantalla**, más un caso a 280 px: hasta v17.10 medía solo a 320 px, que es justo el ancho donde un `@media` oculta el contador de la marca — o sea que la guarda apuntaba al único lugar donde el problema no podía aparecer, y la campana se salía de la pantalla en 360 y 390. El caso de 280 px es el único que ejercita la red de seguridad (`min-width:0` en la marca): ahí nada entra, y lo que se comprueba es que ceda el nombre y **no** se vaya un botón fuera del viewport. El chequeo que más importa es el único que **no mira el DOM**: lee `amet-radar.html` del repo y falla si el correo aparece escrito entero, que es lo que lo dejaría a merced de un scraper |
